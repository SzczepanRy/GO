...
func startScraping(db *database.Queries, concurency int, timeBetweenReq time.Duration) {
	log.Printf("SCRAPING on %v goroutenes every %s duration", concurency, timeBetweenReq)
...
func scrapeFeed(db *database.Queries, wg *sync.WaitGroup, feed database.Feed) {
	...
	_, err := db.MatkFeedAsFeched(context.Background(), feed.ID)
	if err != nil {
		log.Println("error marking fetch feeds")
	}
	...
	for _, item := range rssFeed.Channel.Item {
		...
		if err != nil {
			...
			log.Println("error creating post or post exists")
		}
	}
